...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete OctoCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no OctoCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old OctoCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected OctoCoin 0.7 datadir."
    ;;
  3)
    echo "Detected OctoCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected OctoCoin 0.8 datadir."
    ;;
esac
