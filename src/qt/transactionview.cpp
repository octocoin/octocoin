    ...
    transactionView(0)
{
	...
	QAction *viewOnOctochain = new QAction(tr("Show transaction on OctoCoin Explorer"), this);
	...
	contextMenu->addSeparator();
	contextMenu->addAction(viewOnOctochain);
	...
	connect(viewOnOctochain, SIGNAL(triggered()), this, SLOT(viewOnOctochain()));
}
...
void TransactionView::changedAmount(const QString &amount)
{
    ...
    if(OctoCoinUnits::parse(model->getOptionsModel()->getDisplayUnit(), amount, &amount_parsed))
    {
...
void TransactionView::viewOnOctochain()
{
    QModelIndexList selection = transactionView->selectionModel()->selectedRows();
    if(!selection.isEmpty())
    {
        QString format("http://explorer.octocoin.org/tx/");
