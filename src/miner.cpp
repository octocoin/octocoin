...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OctoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OctoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OctoCoinMiner(CWallet *pwallet)
{
    printf("OctoCoinMiner started\n");
    ...
    try { while (true) {
        ...
        printf("Running OctoCoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OctoCoinMiner terminated\n");
        throw;
    }
}

void GenerateOctoCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OctoCoinMiner, pwallet));
}
